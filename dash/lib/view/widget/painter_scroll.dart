...
class ScrollPaint extends StatelessWidget {
  const ScrollPaint({super.key, required Color color, required double opacity})
      : _color = color,
        _opacity = opacity;
...
class ScrollPainter extends CustomPainter {
  const ScrollPainter({required Color color, required double opacity})
      : _color = color,
        _opacity = opacity;
  ...
  @override
  bool shouldRepaint(covariant CustomPainter oldDelegate) => (oldDelegate as ScrollPainter).color != _color || oldDelegate.opacity != _opacity;
}
