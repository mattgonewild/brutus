...
class GearPaint extends StatelessWidget {
  const GearPaint({super.key, required Color color, required double opacity})
      : _color = color,
        _opacity = opacity;
...
class GearPainter extends CustomPainter {
  const GearPainter({required Color color, required double opacity})
      : _color = color,
        _opacity = opacity;
  ...
  @override
  bool shouldRepaint(covariant CustomPainter oldDelegate) => (oldDelegate as GearPainter).color != _color || oldDelegate.opacity != _opacity;
}
